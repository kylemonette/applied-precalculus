...
def restore_protected_blocks(content, protected_blocks):
    ...
    for i, block in enumerate(protected_blocks):
        content = content.replace(f'___PROTECTED_{i}___', block)
    return content
...
def convert_formatting_to_pretext(content):
    ...
    def replace_bf_braces(text):
        pattern = r'\{\\bf\s+((?:[^{}]|\{[^{}]*\})*)\}'
        while re.search(pattern, text):
            text = re.sub(pattern, r'<alert>\1</alert>', text)
        return text
...
def convert_complete(input_file, output_file):
    ...
    # Protect specified environments first
...
'''
    ...
    # Convert formatting (italics and bold)
    content = convert_formatting_to_pretext(content)
    
    # Restore protected blocks at the end
...
if __name__ == "__main__":
    convert_complete("section-1.2.txt", "section-1.2-pretext.txt")